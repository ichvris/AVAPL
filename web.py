...
async def register(request):
    if not registation:
        return web.Response(text="Rejestracja odblokowana")
    uid, password = utils.bot_common.new_account(app["redis"])
    return web.Response(text=f"Konto stworzone, Twój login to - {uid}, "
                             f"a hasło - {password}")
...
async def main():
    ...
    await runner.setup()
    site = web.TCPSite(runner, "185.229.227.61", int(config["webserver"]["web_port"]))
    await site.start()
